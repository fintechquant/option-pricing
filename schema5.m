...
iftrade2 = iff(abs(DF.Close/DF.Pre_Close-1)<0.095, 1, 0);
...
iftrade2 = iftrade2.modify('Data', [iftrade2.Data(1:end-1,:); ones(1, numel(iftrade2.Dim2))]);
...
% restforb2 = iff(iftrade0 ==0 | ((iftrade1 == 0)&(iftrade2 == 0)), 1, 0);
% restforb2 = subset(restforb2, Dim1, Dim2); restforb2 = iff(isnan(restforb2), 0, restforb2); restforb2 = restforb2.Data;
...
%             ≥ı ºªØ
            ...
            KEEP_1 = KEEP(2:end,:);
            KEEP_3 = KEEP(1,:) .* double(restforb(t0,:)==1);
...
%                     disp(rankmin)
                    if vKEEP - tempbuy > 0
                        KEEP_1(:,rankInidx) = multBuy_1(:,rankInidx)* vIndex/window_days/length(rankInidx);
                        vKEEP = vKEEP - tempbuy;
%                         disp(['change: -' num2str(tempbuy)])
%                         disp(['vKEEP: ' num2str(vKEEP)])
                    else
                        KEEP_1(:,rankInidx) = vKEEP/(window_days-1)/length(rankInidx) ;
